...
class AttendanceService {
  ...
  Future<void> _recordAttendanceActivity(List<AttendanceModel> attendances) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final activities = prefs.getStringList('recent_attendance_activity') ?? [];
      
      for (final attendance in attendances) {
        final activity = '${attendance.studentId}|${attendance.status}|${attendance.attendanceTime.toIso8601String()}|${DateTime.now().millisecondsSinceEpoch}';
