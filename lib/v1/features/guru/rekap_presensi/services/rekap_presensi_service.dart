...
class RekapPresensiService {
  ...
  // Simplified method untuk query attendance dengan schedule_id langsung dengan timeout
  Future<Map<String, dynamic>> getAttendanceByScheduleId({
    ...
    try {
      ...
      print('🔍 Querying attendance by schedule_id with TIMEOUT:');
      ...
      Timer(Duration(seconds: 8), () {
        if (!cancelToken.isCancelled) {
          cancelToken.cancel('Request timeout after 8 seconds');
        }
      });

      // Try simplified approach with timeout
      ...
      if (response.statusCode == 200) {
        ...
        print('📊 No attendance data found (404) - returning empty result');
        return {
          'attendance_data': [],
          'additional_info': {
            'id': '1',
            'nama_mata_pelajaran': 'No Data Available',
            'kelas': 'No Class',
            'guru': [
              {'id': '1', 'nama_guru': 'Teacher'},
            ],
      ...
      if (e.response?.statusCode == 404) {
        print('📊 No attendance data found (DioException 404)');
        return {
          'attendance_data': [],
          'additional_info': {
            'id': '1',
      ...
      print('❌ Dio error in getAttendanceByScheduleId: ${e.response?.data}');
      throw Exception(
      ...
      return {
        ...
        'additional_info': {
          'id': '1',
          'nama_mata_pelajaran': 'Error Loading',
          'kelas': 'Error',
          'guru': [
            {'id': '1', 'nama_guru': 'Teacher'},
          ],
